            ...
            <div class="card h-100 border-0" style="height: 230px">
              ...
              <p class="small fw-bold m-0 text-muted" style="font-size: 10px;">
                @foreach($provinsis as $provinsi)
                  @if($sportainment->provinsi == $provinsi->id_provinsi){{ $provinsi->nama_provinsi }}, @endif
                @endforeach
                @foreach($kabupatens as $kabupaten)
                  @if($sportainment->kabupaten_kota == $kabupaten->id_kabupaten){{ $kabupaten->nama_kabupaten }}, @endif
                @endforeach
                @foreach($kecamatans as $kecamatan)
                  @if($sportainment->kecamatan == $kecamatan->id_kecamatan){{ $kecamatan->nama_kecamatan }}@endif
                @endforeach
              </p>
            </div>
          </a>
